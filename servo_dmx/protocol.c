...
uint16_t CRC16(uint16_t crc_accum, uint8_t *data_blk_ptr, uint16_t data_blk_size){ // crc_accum = 0!
    uint16_t i, j;
    ...
    {
        i = ((uint8_t)(crc_accum >> 8) ^ data_blk_ptr[j]) & 0xFF;
...
void USART1_RXHandeler(uint8_t* data, uint32_t rx_len){
  ...
  if (CRC16(0, data, rx_len-2) != crc) return;
